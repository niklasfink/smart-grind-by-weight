...
The device web page also exposes the same practical settings as the touchscreen menu: grind mode, purge mode and amount, freshness, coast compensation, automation toggles, basket detection, logging, brightness, WiFi startup, screensaver timing, screensaver image upload, and OTA upload. Each setting includes inline help text, and settings are stored in the existing NVS preference namespaces so the web UI does not duplicate settings storage.
